...
class CodeValidator:
    ...
    def apply_diff_patch(self, diff_text: str, commit_message: Optional[str] = "Apply LLM patch") -> Tuple[bool, str]:
        """
        Try to apply a unified diff via `git apply` then commit.
        Returns (ok, output).
        """
        ...
        try:
            ...
            if rc != 0:
                ...
                if rc2 != 0:
                    logger.debug("git apply and patch both failed")
                    return False, out + err + "\n" + out2 + err2
            # If apply succeeded, commit
            ok_commit, commit_out = self.commit_all(commit_message)
            if not ok_commit:
                # commit might fail if no changes staged
                return True, "Applied patch but commit failed or nothing to commit: " + commit_out
            return True, commit_out
        except Exception as e:
            logger.exception("apply_diff_patch failed: %s", e)
            return False, str(e)
        finally:
